...
; -> 오토 헬파시 헬파쏘고 마비 돌리기  alt + c : 기본 사용 (ParalysisAfterAutoHellFire 을 변경)
...
;g중독 돌리기 c 마비 돌리기, v 4방향 활력+마비(마비 갱신) b첨첨자동사냥, shift+b 중독자동사냥 alt+b 중독쩔
...
;원샷헬파이어에 사용되는 변수
;HellFireForAuto() 에서 헬파이어를 시전하면 카운트가 1 올라가고 OneShotHellFire() 에서는 카운트가 1이면 종료
;즉 본인이나 다른 유저가 타겟잡혔을 땐 헬파이어를 시전하지 않고 break되므로 카운팅이 올라가지 않는다.
;문제는 한 번 시전하면 OneShotHellFireCount를 올리는데 이걸로 마비 안 돌리게 하면 hellFireForAuto()에서 마비를 돌릴 수 없게 된다.
;그래서 변수 하나를 더추가 해줘야겠다. isOneShotHellFire 가 true고 OneShotHellFireCount > 0 이면 마비 안 돌리는 걸 HellFireHunt() 마무리에 추가
global OneShotHellFireCount := 0
global isOneShotHellFire := false
...
;한 방 헬파 쓸 때도 마비 돌리기를 안 쓰게 해주기 위해 HellFireForAuto() 마무리에 관련 변수 isONeShotHellFire가 true이고
;OneShotHellFireCount > 0  일 때  HellFireForAuto()에서 마무리에 마비돌리기를 하지 않는다.

;isOneShotHellFire는 OneShotHellFire() 에서 true로 초기화 해주고 OneShotHellFireCount는 HellFireForAuto()에서
;실제로 헬파를 쏘고 공증을 거친 뒤 원래라면 자힐을 해주는 위치에서 OneSHotHellFireCount++를 해줬다.
;즉 헬 한 방만 쓰기 위해 변수를 2개 만들었다. OneSHotHellFireCount와 isOneShotHellFire 이렇게.
    ...
    Loop ,20 { ;적당한 반복수
        ...
        if(isFullMana) { ; 풀마나 상태일 때 (공력증강 or 헬파 씹힘)
            if(isRefreshed && waitingHellFireCount > 0) { 
                ...
                OneShotHellFireCount++   ;실제로 헬파 쏘고 공증까지 거쳐온 것이므로 헬파 한 방 쐈다고 볼 수 있다.
                CustomSleep(20)
                Break
            } else { ;풀마나인데 공증 거친 것이 아니면 헬파 시전 안 된 것이므로 다시 루프 반복되면서 자힐로 안 가고 헬파로 다시 온다.            
