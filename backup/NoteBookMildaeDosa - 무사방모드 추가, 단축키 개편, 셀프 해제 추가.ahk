...
;그래서 지금은 shift 조합으로 사용하는  시력회복, 퇴마주, 파혼, 해독 단일 시전 등을 ctrl 조합으로 하면 어떨까 싶다.
;단 이때 ctrl + w같이 시력회복 돌리기 (다른 유저가 필요할 때)를 사용하므로 일반 시력회복 시전은 단축키를 바꿔야 할 것이다. 
;탭탭 대상에게 사용하지 않을 무력화 같은 건 shift + q에 해놔도 괜찮을 것 같다.
;일단은 시력회복, 파혼은 이거 걸리는 사냥터 갈 때는 토글로 on/off 하고 해독은 shift + z로 시전해야되므로 힐에 넣어주면 틱 밀리기 때문에
;중독 얼마 안 까지는 건 무시해도 되겠지만 해제 해주려면 ctrl + f로 수동 해제제
;공력주입같은 경우 탭탭대상 시전은 t,일반시전은 shift + t가 있으므로 ctrl + t가 비는데 여기에 퇴마주를 넣어주자.
;퇴마주는 탭탭대상에게 주로 쓰는 게 아니라 주술이랑 할 떄 저주 잘못 넣었을 때 풀어주는 용도이므로 ctrl + t에 넣고
;나중에 ctrl + e,r,t 빈도에 따라 또 바꿔주던가 하자.

;단축키 설정
;wasd 이동
;f2 자신선택 및 동동주   f3 자신선택 및 말타기 및 중지(현재 동작중인 함수 종료.)
;1 그냥 힐(탱커모드시 공력증강 빈도 감소소) 백호의히원 반복 (무빙가능)   2 정지   3 추적혼힐  4 제자리 혼힐(탱커모드시 공력증강 빈도 감소),5 미정
;q 금강(힐 중에는 자동사용)  e 짧은 혼힐 돌리기, f 적당한 혼힐 돌리기(탱커모드시 더 많이) c 긴 혼마 돌리기, r 선택혼, t탭탭대상 공력주입
;v: 탭탭대상 보호+무장  g 입력대기 부활(space로 선택. 보호 + 힐3틱.  무사방 모드 상관없이 시력회복,파혼,해독 추가시전 ->써보고 롤백)

;ctrl + q : 본인 각종회복(시력회복,파혼술,해독 + 보호)// ctrl + w : 시력회복 돌리기// ctrl + e : 파혼술 //ctrl + r:시력회복 //ctrl + t : 퇴마주주
; ctrl + a:신령지익진// ctrl + s : 상태창// ctrl + d : 파력무참진  //ctrl + f : 해독독
...
; F2 Home(타겟박스 있을 때 적용) + 동동주(타겟박스 없을 때만 마심)  F3 home + StopLoop(루프 중단변수)  ->일반 키보드에선 F1, F2키
...
;e매우짧혼, f중간혼 (e왕 f는 힐 돌리면서 혼마 3개씩 돌리기를 짧게, 길게. 탱커모드에서는 f로는 좀 더 많이 돌리기) c 혼마만 돌리기
;r 선택혼 ,t공주 , g 입력대기(수동부활 등)  shift + g 탭탭부활 본인부활 후 다시 탭탭  v보무 
...
;b유령부활에 못 찾았을 경우 마지막 ese에 유령부활2 를 넣어줬다.
...
;입력대기  (c키와 esc는 취소, e키 ,좌클릭 -> 수동 부활 등)
g:: 
InputWaiting()
return
...
InputWaiting() {    
    ...
    ;키 입력 대기 (10초 타임아웃)
    Input, UserInput, V L1 T10, {space}{ESC}
    CustomSleep(20)
