...
;StopLoopCheck()에 걸려서 Exit로 인해 종료시 변수 설정을 리셋 못 시켜주는 건 여기서 리셋을 반드시 해줘야 다음 함수시 꼬이지 않는다.
StopLoopCheck() {
    if (StopLoop)
        {            
            SendInput, {Esc}
...
if(isTankerModOn) {
    Honheal(6,1) ; 탱커모드에서는 힐 빡세게 안 하니까 힐틱 구애받지 않고 혼 좀 더 돌림림
    return
}
HonHeal(3,2)
return
...
HonmaLeftForHonHeal(count) {
    Loop, %count% { ; 후딜 20~30짜리들 조금 깎아봤다. 후딜 10으로 하니 그 다음키가 씹히는 경우가 생겼다. (혼마 없이 왼쪽 눌러져서 이동됨)
        ...
        CustomSleep(30) ;원래 30
        SendInput, { enter }
        CustomSleep(50)  ;후딜 90. 꼬임 방지로 ESC 넣고 후딜 나눴음. 60도 괜찮아서 60을 40, 20으로 나눔. 씹혀서 70
        SendInput, {Esc}
        CustomSleep(20)  ;
    }
return    
