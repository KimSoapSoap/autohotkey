...
; -> 선비족 : alt + e 기본 false(isAtSunbi를 변경) -> 입력대기 헬파, 자동헬파에서 마비 절망도 안 쓰고 중독을 건다
...
;단축키
; F2 Home + 동동주(타겟박스O-> Home적용, 타겟박스x ->동동주마시기 적용)  F3 말타기 + StopLoop(루프 중단변수)  ->일반 키보드에선 F1, F2키
...
;a저주 돌리기,  s 입력대기(아래 자세히), d(절망 + 저주-> 마비 건 몹에 사용할.),  f 4방향 마비 , shift + f 4방향 마비 저주, 
...
;d는 q,w,e,r(r키는 애매) 마비 절망 활력을 편하게 쓰기 위해 Enter키로 써도 될 듯 (절망 걸고 저주는 한 번 싹 돌리면 되니까)
;그러면 왼손 검지에 부담 가려나? 일단 입력대기 헬파에 자동저주 있으니까 전체 저주 안 돌리고 일단 d를 절망 + 저주 시전(q같은 걸로 타겟로 대상지정 후 d)


;입력대기
; s 입력대기 -> 입력대기중 동작키를 누른다.
; d :저주+헬파(alt+r토글시 시전 전 r키 on/off) 공증 자힐(자힐도 alt+s 토글로 on/off). 시전이 되면 종료. 안 되면 20회 반복 후 종료(초당 2번정도)
; a 삼매진화   q타겟 포함 상하좌우 마비, w 타겟 포함 상하좌우 절망  e타겟 포함 상하좌우 저주(활력을 해야될텐데 잘 안 쓸 듯)
; r 타겟 포함 상하좌우 절망 + 저주
; shift + e 타겟 포함 상하좌우 활력

; -> 입력대기에서 shift 조합같은 것은 실제로 가능하지가 않다. 단일키 입력만 가능한데 입력대기에서 사용하지 않는 y, u , i, h, j k ... 등의 키를 대체해서 입력받고
;    사용할 핫키(shift + e  ctrl +  등등)에서 send, y 이런식으로 해서 우회적으로 사용한 것이다
...
;입력대기 스킬 사용 대기중일 때 입력대기 스킬이 아닌 키를 사용할 때는 함수 자체에 CancelInput := true를 다 넣어줄까?
;어차피 사용 대기중일 때만 체크하는 것이므로
...
;무사처럼 마비, 절망을 안 건다. 다만 마력 높아지기 전까지 중독을 돌려준다.
;오토헬파와 헬파원샷(HellFireForAuto() 에서 조정) 입력대기 헬파에서 조정해준다.
global isAtSunbi := false
...
; 전역 변수: 현재 Always On Top 상태인 창의 핸들을 저장
global currentAOTWindow := ""
...
!p::  ; 현재창을 항상 최상단으로하는 토글. 우측 템창에 영상 시청을 위함
        ; 현재 활성 창의 핸들 가져오기
        WinGet, hwnd, ID, A

        ; 이전에 설정된 창이 있고 현재 창과 다르다면, 이전 창의 Always On Top 끄기
        if (currentAOTWindow && currentAOTWindow != hwnd)
            WinSet, AlwaysOnTop, Off, ahk_id %currentAOTWindow%
    
        ; 현재 창의 Always On Top 토글
        WinSet, AlwaysOnTop, Toggle, ahk_id %hwnd%
    
        ; 현재 창의 상태 확인 (WS_EX_TOPMOST = 0x8)
        exStyle := DllCall("GetWindowLong", "UInt", hwnd, "Int", -20)
        if (exStyle & 0x8)
            currentAOTWindow := hwnd
        else if (currentAOTWindow == hwnd)
            currentAOTWindow := ""
...
;선비족 사냥. 마비가 안 걸리지만 절망을 쓰지 않는 곳 -> 마력 낮을 때 중독을 돌린다.(마력 높아지면 중독 뺀다)
!e::
ToggleSunbiHunt()
return
...
CancelInput := true ; 입력대기중 사용시 입력대기 취소
HellFireHunt()
return
...
CustomSleep(180) ;쉬프트 누르고 뗄 때 후딜(짧으면 함수 실행될 때 shift 조합으로 입력돼서 오작동)
if(isWaiting) {
    Send, y ; 입력대기 shift 조합이 안 돼서 입력대기에 사용하지 않는  y, u, i 등의 키를 입력해서 사용
    return
}
...
; 선비족 사냥인지 아닌지 설정.(마비가 안 걸리지만 절망도 사용하지 않을 곳 -> 수동으로 절망 사용할 곳 + 검객 한 방 뜨기 전까지 마비 절망대신 중독)
ToggleSunbiHunt() {
    if (isAtSunbi) {
        ; 선비족 사냥 끄기
        isAtSunbi := false
...
FourWayDespair() {  
    ; 7 절망
    FourWayMagic(7, "Left", 1)
    FourWayMagic(7, "Right", 1)
    FourWayMagic(7, "Up", 1)
    FourWayMagic(7, "Down", 1)   
    SendInput, {Esc}
    CustomSleep(20)
    return
}
...
; 4방향 마법인데 내 기준이 아니라 선택했던 타겟박스 기준. 그렇기 때문에 가운데 자리도 시전한다.
;일단 써보고 씹히면 후딜 좀 늘린다.
FourWayMagicAtTargetBox(key) { 
    ;입력대기에서 사용할 때 동작키 누를 때 esc랑 동시입력돼서 기존에 열었던 저주 타겟박스가 안 닫히면 처음 꺼 씹혀서 후딜 넣어둠
    ;시작시 후딜을 20~30 할 때는 입력대기에서 동작키 누를 때 esc가 씹혀서 저주 타겟박스가 안 사라지고 걸리지 않습니다가 뜨고
    ;첫 스킬이 한 번 씹혔다.
...
;입력대기 스킬 사용 대기중일 때 입력대기 스킬이 아닌 키를 사용할 때는 CancelInput := true를 다 넣어줄까?
;어차피 사용 대기중일 때만 체크하는 것이므로

;우선 4방위 마비 돌리기인 f:: 에만 CancelInput := true를 넣어줬는데 이는 입력대기중 키 연계시 f키는 포기한 것. 그럴만한 것이 4방위 마비라서 급할 때 유용

;입력대기 이후 동작키에 shift 조합을 사용하려고 했는데 따로 +e나 E를 받아서 사용은 불가능했다.
;그래서 원한다면 우회적으로 예를들어서 입력대기에 사용하지 않는 y u i o p h j k l  이런 키들을 입력받고 +e 핫키에 isWaiting일 경우 y키를 Send하는 등으로 사용
...
InputWaiting() {    
    ...
    ; Enter와 d 키 입력 대기 (10초 타임아웃), c(esc입력되게 함)와 esc는 취소.
    ; 이후 다른 스킬들 늘어날 수 있음.
    Input, UserInput, V L1 T10, {d}{a}{ESC}{q}{w}{e}{r}{y}
    CustomSleep(20)
    ...
    if (ErrorLevel = "EndKey:d") { ; 입력대기중 d키 눌렀을 때 헬파이어 -> 입력대기중에는 isWaiting 변수를 활용하여 d키 입력시 o가 입력되게 해서 연동. 마우스클릭도 o가 입력되게.
        ...
        Loop ,20 { ;루프 횟수없으면 만약 말에서 내린 상태에서 s->d를 해버릴 때 말에 타버리면 말탄 상태에서 무한 공증시도를 하게 된다.
            ...
            if(isFullMana) { ; 풀마나 상태일 때 (공력증강 or 헬파 씹힘)
                    ...
                    if(waitingHellFireCount >0) { ;첫 헬파쐈는데 공증 없이 다시 풀마나로 왔다는 것은 쿨타임이라는 것이다. 쿨일시 마비(혹은 절망) 쏘고 헬파
                        ...
                        loop, 1 { ;마비 100퍼 같아서 2에서 1로 변경
                            if(isAtForest) {
                                ...
                                ;검객 한 방 뜰 마력이 아니라서 중독 넣어줌(마비 절망은 쓰지 않는다.)
                                SendInput, {9} ;중독
                                CustomSleep(30)
                                SendInput, {Enter} 
                                CustomSleep(30)
                ...
                ;기존 로직으로는 풀마나가 아닌 것이 헬파를 쏘고 페이백 받고 내려왔는지 다른 거 쓰느라 마력 소모를 하고 왔는지 알 수 없없다.
                ;그래서 우선 풀마나 아닐시 공증을 할지 그대로 헬파를 쓸지 정해야되는데 공증 후 풀마나 만들고 헬파 쏘는 방향으로 정했다.
                ;그렇기 때문에 풀마나 아닌 상태로 내려오면 바로 공력증강 시도하면 된다. 그런데 문제는 빠르게 접근하는 몬스터를 타겟으로 했을 때는
                ;공력증강 시도하기 전에 마비나 절망으로 묶어놔야 한다. 그래서 헬파이어를 시전하고 왔는지 아직 시전 안 했는데 풀마나 아니라서 왔는지
                ;알 수 없다.
                ; 즉 헬파를 썼는지 알 수 있게  waitingHellFireCount 변수를 만들고 0부터 시작해서 헬파 시전시 1씩 카운팅 올라가게 했다. 0이면 아직 시전 안 됨.
                ;즉 여기에 왔다는 것은 풀마나가 아닐 때이고 이떄 waitingHellFireCount 변수가 0이면 헬파를 시전 안 했으므로
                ;공증을 하기 전에 다가오는 적을 마비나 절망을 걸기 위해 조건을 넣었다.
                ...
                if(waitingHellFireCount==0) {
                    ...
                    if(isAtForest) {
                        ...
                        SendInput, {6} ;마비(100% 확률인 것 같으므로 일단 1회 시전.)
                        CustomSleep(30)
                        SendInput, {Enter} 
                        CustomSleep(30)
                    }
    ...
    ;---------------------입력대기중 삼매진화------------------------------------------------------
        ...
        CustomSleep(80) ; 후딜 70이었는데 가끔 삼매하고 무반응이길래 후딜 110으로 늘려봤다.
        SendInput, {Esc} ; 꼬임방지. 후딜 110을 80, 30으로
        CustomSleep(30)
        ...
        } else if(isManaZero) { ; 마나가 0이면 삼매진화 시전된 것
    ...
    } else if (ErrorLevel = "EndKey:q") { ; 타겟박스 기준 4방향 마비
        FourWayMagicAtTargetBox(6) ; 6 마비

    } else if (ErrorLevel = "EndKey:w") { ; 타겟박스 기준 4방향 절망
        FourWayMagicAtTargetBox(7) ; 7 절망        

    } else if (ErrorLevel = "EndKey:e") { ; 타겟박스 기준 4방향 활력 -> 활력은 사실 4방향 활력 필요 없긴 하다. 일단 저주
        FourWayMagicAtTargetBox(4) ; 4 저주

    } else if (ErrorLevel = "EndKey:r") { ; 타겟박스 기준 4방향 절망 + 저주
        FourWayTwoMagicsAtTargetBox(7,4) ; 7 절망   4 저주

    } else if (ErrorLevel = "EndKey:y") { ; 타겟박스 기준 4방향 활력  (shift + e 핫키에 Send, y)
        FourWayMagicAtTargetBox(8) ; 8 활력
    ...
    Loop ,20 { ;적당한 반복수
        ...
        if(isFullMana) { ; 풀마나 상태일 때 (공력증강 or 헬파 씹힘)
            if(isRefreshed && waitingHellFireCount > 0) { 
                ...
                HellFireForAutoCount++   ;실제로 헬파 쏘고 공증까지 거쳐온 것이므로 헬파 한 방 쐈다고 볼 수 있다. 원샷헬파용 카운팅
                CustomSleep(20)
                Break
            } else { ;풀마나인데 공증 거친 것이 아니면 헬파 시전 안 된 것이므로 다시 루프 반복되면서 자힐로 안 가고 헬파로 다시 온다.            
            ...
            if(waitingHellFireCount==0) {
                ...
                if(isAtForest) {
                    ...
                    SendInput, {6} ;마비 (100퍼 같아서 1회만 시전)
                    CustomSleep(30)
                    SendInput, {Enter} 
                    CustomSleep(30)
                }
...
;HellFireForAutoCount 변수가 OneShotHellFire() 즉, 한 방만 쏠 때 사용하는 헬파가 시전됐는지 실제 카운팅이므로
