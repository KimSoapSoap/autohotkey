...
;q 금강(힐 중에는 자동사용)  e 짧은 혼힐 돌리기, f 적당한 혼힐 돌리기(탱커모드시 4방향 혼혼) c 긴 혼마 돌리기, r 선택혼, t탭탭대상 공력주입
;v: 탭탭대상 보호+무장  g 입력대기 부활(space로 선택. 보호 + 힐3틱.  무사방 모드 상관없이 시력회복,파혼,해독 추가시전 ->써보고 롤백)

;ctrl + q : 본인 각종회복(시력회복,파혼술,해독 + 보호)// ctrl + w : 시력회복 돌리기
;ctrl + e : 파혼술 //ctrl + r : 해독 // ctrl + t:시력회복(시력회복 돌리기 있어서 손이 먼 t에 했다)
; ctrl + a:신령지익진// ctrl + s : 상태창// ctrl + d : 파력무참진  //ctrl + f : 퇴마주
...
HonHeal(HonCount, LoopCount) { 
    ...
    loop, %LoopCount%
    {
        ...
        Loop, %HonCount% {
            ...
            CustomSleep(30)
            SendInput, { enter }
            CustomSleep(40)  ;후딜 90. 꼬임 방지로 ESC 넣고 후딜 나눴음. 60 30으로 나눔.  왜그런지 혼마 씹히는 경우가 생겨서 70 20으로하니 ㅇㅋ
            SendInput, {Esc}
            CustomSleep(20) 
            ;SendInput, {6} ;금강불체. ;혼마 마지막이 씹혀서 금강을 밖으로 빼봤다. 빼니까 안 씹힘
        }
...
ChaseHonHeal() {  ;추적 혼힐
    ...
    loop, 330 ; 안전장치로 카운트 걸어둠. 어차피 맵 넘어갈 때 껐다 켜야되므로 걸어두는 것이 낫겠다. 거의 초당 1회다.
    {
        ...
        Loop, 3 {
            ...
            CustomSleep(20) ; 
            ;SendInput, {6} ;금강불체. ;혼마 마지막이 씹혀서 금강을 밖으로 빼봤다. 빼니까 안 씹힘힘
        }
        SendInput, {6} ;금강불체.
        CustomSleep(20)
...
StandingHonHeal() { ;제자리 혼힐
    ...
    loop, 630 ;안전장치로 카운트 걸어둠. 10분 30초. 추적혼힐은 맵 넘어가야돼서 5분 안팍이고 스탠딩은 시즈모드라서 10분쯤
    {
        ...
        Loop, 3 {
            ...
            CustomSleep(30)
            SendInput, { enter }
            CustomSleep(40)  ;후딜 90. 꼬임 방지로 ESC 넣고 후딜 나눴음. 60 30으로 나눔. 
            SendInput, {Esc}
...
InputWaiting() {    
    ...
    ;키 입력 대기 (10초 타임아웃)
    Input, UserInput, V L1 T10, {space}{ESC}{e}{r}{t}{f}
    CustomSleep(20)
